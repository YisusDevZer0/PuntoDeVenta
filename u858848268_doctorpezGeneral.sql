...
-- Tiempo de generación: 14-01-2026 a las 19:37:42
...
-- Procedimientos
--
CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `GenerarNotificaciones` ()   BEGIN
    -- Variable para sucursal por defecto si es necesario
    DECLARE id_sucursal_default INT DEFAULT 1; -- Ajusta según tu caso
    
    -- 1. Notificaciones de inventario bajo
    INSERT INTO Notificaciones (Tipo, Mensaje, Fecha, SucursalID, Leido)
    SELECT 
        'inventario' as Tipo,
        CONCAT('El producto ', s.Nombre_Prod, ' tiene stock bajo (', s.Existencias_R, ') en sucursal ', 
               COALESCE(suc.Nombre_Sucursal, CONCAT('ID:', s.Fk_sucursal))) as Mensaje,
        NOW() as Fecha,
        s.Fk_sucursal as SucursalID,
        0 as Leido
    FROM Stock_POS s
    LEFT JOIN Sucursales suc ON s.Fk_sucursal = suc.ID_Sucursal
    WHERE s.Existencias_R <= s.Min_Existencia
    AND s.Existencias_R > 0
    AND NOT EXISTS (
        SELECT 1 FROM Notificaciones n
        WHERE n.Tipo = 'inventario'
        AND n.Mensaje LIKE CONCAT('%', s.Nombre_Prod, '%')
        AND DATE(n.Fecha) = CURDATE()
        AND n.SucursalID = s.Fk_sucursal
    )
    LIMIT 50;
    
    -- 2. Notificaciones de productos próximos a vencer
    INSERT INTO Notificaciones (Tipo, Mensaje, Fecha, SucursalID, Leido)
    SELECT 
        'caducidad' as Tipo,
        CONCAT('El producto con ID ', h.ID_Prod_POS, ' lote ', h.Lote, ' caduca el ', 
               DATE_FORMAT(h.Fecha_Caducidad, '%d/%m/%Y'), ' en ', 
               COALESCE(suc.Nombre_Sucursal, CONCAT('Sucursal ID:', h.Fk_sucursal))) as Mensaje,
        NOW() as Fecha,
        h.Fk_sucursal as SucursalID,
        0 as Leido
    FROM Historial_Lotes h
    LEFT JOIN Sucursales suc ON h.Fk_sucursal = suc.ID_Sucursal
    WHERE h.Fecha_Caducidad BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 15 DAY)
    AND h.Existencias > 0
    AND NOT EXISTS (
        SELECT 1 FROM Notificaciones n
        WHERE n.Tipo = 'caducidad'
        AND n.Mensaje LIKE CONCAT('%ID ', h.ID_Prod_POS, '%')
        AND n.Mensaje LIKE CONCAT('%lote ', h.Lote, '%')
        AND DATE(n.Fecha) = CURDATE()
        AND n.SucursalID = h.Fk_sucursal
    )
    LIMIT 50;
    
    -- 3. Notificaciones de corte de caja pendiente
    INSERT INTO Notificaciones (Tipo, Mensaje, Fecha, SucursalID, Leido)
    SELECT 
        'caja' as Tipo,
        CONCAT('La caja #', c.ID_Caja, ' de ', c.Empleado, ' lleva abierta desde ', 
               DATE_FORMAT(c.Fecha_Apertura, '%d/%m/%Y'), ' en ', 
               COALESCE(suc.Nombre_Sucursal, CONCAT('Sucursal ID:', c.Sucursal))) as Mensaje,
        NOW() as Fecha,
        c.Sucursal as SucursalID,
        0 as Leido
    FROM Cajas c
    LEFT JOIN Sucursales suc ON c.Sucursal = suc.ID_Sucursal
    WHERE (c.Estatus = 'Abierta' OR c.Estatus = 'abierta' OR c.CodigoEstatus = '1')
    AND DATEDIFF(CURDATE(), c.Fecha_Apertura) >= 1
    AND NOT EXISTS (
        SELECT 1 FROM Notificaciones n
        WHERE n.Tipo = 'caja'
        AND n.Mensaje LIKE CONCAT('%caja #', c.ID_Caja, '%')
        AND DATE(n.Fecha) = CURDATE()
        AND n.SucursalID = c.Sucursal
    );
    
    -- 4. Notificaciones de ventas altas
    INSERT INTO Notificaciones (Tipo, Mensaje, Fecha, SucursalID, Leido)
    SELECT 
        'venta' as Tipo,
        CONCAT('Se registró una venta por $', FORMAT(v.Total_VentaG, 2), ' por ', v.AgregadoPor, 
               ' en ', COALESCE(suc.Nombre_Sucursal, CONCAT('Sucursal ID:', v.Fk_sucursal))) as Mensaje,
        NOW() as Fecha,
        v.Fk_sucursal as SucursalID,
        0 as Leido
    FROM Ventas_POS v
    LEFT JOIN Sucursales suc ON v.Fk_sucursal = suc.ID_Sucursal
    WHERE v.Total_VentaG > 5000
    AND v.Fecha_venta = CURDATE()
    AND v.Estatus = 'Vigente'
    AND NOT EXISTS (
        SELECT 1 FROM Notificaciones n
        WHERE n.Tipo = 'venta'
        AND n.Mensaje LIKE CONCAT('%venta por $', FORMAT(v.Total_VentaG, 2), '%')
        AND DATE(n.Fecha) = CURDATE()
        AND n.SucursalID = v.Fk_sucursal
    )
    GROUP BY v.Folio_Ticket
    LIMIT 20;
    
    -- 5. Recordatorio del sistema (siempre funcionará)
    INSERT INTO Notificaciones (Tipo, Mensaje, Fecha, SucursalID, Leido)
    VALUES
    ('sistema', 'Se ha ejecutado correctamente la generación de notificaciones automáticas', NOW(), id_sucursal_default, 0);
END$$

CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `LimpiarLogsChecador` (IN `dias` INT)   BEGIN
    DELETE FROM logs_checador 
    WHERE created_at < DATE_SUB(NOW(), INTERVAL dias DAY);
END$$

CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `sp_chat_estadisticas` (IN `usuario_id` INT)   BEGIN
    SELECT 
...
CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `sp_chat_limpiar_mensajes_antiguos` (IN `dias_antiguedad` INT)   BEGIN
...
CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `sp_estadisticas_recordatorios` (IN `fecha_inicio` DATE, IN `fecha_fin` DATE)   BEGIN
    SELECT 
...
CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `sp_limpiar_logs_recordatorios` ()   BEGIN
...
CREATE DEFINER=`u858848268_devpezer0`@`127.0.0.1` PROCEDURE `sp_obtener_recordatorios_pendientes` ()   BEGIN
