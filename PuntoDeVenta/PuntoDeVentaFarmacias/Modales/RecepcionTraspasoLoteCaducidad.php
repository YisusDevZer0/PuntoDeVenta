...
if ($id > 0 && isset($conn) && $conn) {
    ...
    $stmt->close();
    // Farmacias: solo permitir recibir traspasos destinados a la sucursal del usuario
    if ($traspaso && (int)$traspaso['Fk_SucursalDestino'] !== $sucursal_usuario) {
        $traspaso = null;
    }
}
?>
...
$(function() {
    ...
    function actualizarDistribucion() {
        ...
        if (cantidadPrincipal > 0 && cantidadAdicional > 0) {
            ...
            if (cantidadPrincipal <= 0) $('#cantidad_lote_principal').addClass('is-invalid');
            if (cantidadAdicional <= 0) $('#cantidad_lote_adicional').addClass('is-invalid');
        }
    }

    $('#lote').on('change keyup', function() { $('#nombre_lote_principal').text($(this).val() || '-'); });
    $('#nombre_lote_principal').text($('#lote').val() || '-');

    $cantidadRecibida.on('change keyup', function() {
        if ($motivoDiferencia.val() === 'otro_lote') return;
    ...
    $('#cantidad_lote_principal, #cantidad_lote_adicional').on('change keyup', function() {
        if ($motivoDiferencia.val() === 'otro_lote') actualizarDistribucion();
    });
    ...
    $motivoDiferencia.on('change', function() {
        ...
        if (motivo === 'no_completo') {
            ...
            $('#observaciones').val(obsActual && !obsActual.includes('No llegó completo') ? obsActual + '\n' + nuevaObs : (nuevaObs || obsActual));
        } else if (motivo === 'otra_razon') $mensajeOtraRazon.show();
    });
    ...
    $('#cantidad_lote_adicional').on('change keyup', function() {
        ...
        $(this).toggleClass('is-invalid', cantidadTotal - cantidadAdicional <= 0);
    });

    $('#formRecepcionTraspasoLote').on('submit', function(e) {
        e.preventDefault();
        var motivo = $motivoDiferencia.val();
        var cant = motivo === 'otro_lote' ? (actualizarDistribucion(), parseInt($('#cantidad_recibida_hidden').val(), 10) || 0) : parseInt($('#cantidad_recibida').val(), 10) || 0;
        if (cant < 1) { Swal.fire('Atención', 'La cantidad recibida debe ser mayor a 0', 'warning'); return; }
        if (motivo === 'otro_lote') {
            var cantidadPrincipal = parseInt($('#cantidad_lote_principal').val(), 10) || 0;
            var cantidadAdicional = parseInt($('#cantidad_lote_adicional').val(), 10) || 0;
            var loteAdicional = $('#lote_adicional').val().trim();
            var fechaAdicional = $('#fecha_caducidad_adicional').val();
            if (cantidadPrincipal < 1) { Swal.fire('Atención', 'Debe ingresar la cantidad del lote principal', 'warning'); return; }
            if (cantidadAdicional < 1) { Swal.fire('Atención', 'Debe ingresar la cantidad del lote adicional', 'warning'); return; }
            if (!loteAdicional || !fechaAdicional) { Swal.fire('Atención', 'Debe completar todos los campos del lote adicional', 'warning'); return; }
            if ($('#lote').val().trim() === loteAdicional) { Swal.fire('Atención', 'El lote adicional debe ser diferente al lote principal', 'warning'); return; }
            cant = cantidadPrincipal + cantidadAdicional;
        } else if (cant !== cantidadEnviada && !motivo) {
            Swal.fire('Atención', 'Debe seleccionar el motivo de la diferencia en la cantidad', 'warning');
            return;
        }
        if (!$('#lote').val().trim()) { Swal.fire('Atención', 'Ingrese el lote', 'warning'); return; }
        if (!$('#fecha_caducidad').val()) { Swal.fire('Atención', 'Ingrese la fecha de caducidad', 'warning'); return; }
...
<?php else: ?>
<p class="alert alert-danger mb-0">Traspaso no encontrado, ya fue recibido o no corresponde a su sucursal.</p>
<?php endif; ?>
