...
<head>
    <meta charset="utf-8">
    <title>Proveedores <?php echo $row['Licencia']?></title>
...
<script src="js/ControlDeProveedores.js"></script>
   ...
   $(document).ready(function() {
    ...
    $(document).on("click", ".btn-edita", function() {
      console.log("Botón de cancelar clickeado para el ID:", id);
        var id = $(this).data("id");
        $.post("<?php echo BASE_URL; ?>ControlYAdministracion/Modales/EditaProveedor.php", { id: id }, function(data) {
            $("#FormCajas").html(data);
    ...
    $(document).on("click", ".btn-elimina", function() {
        var id = $(this).data("id");
        $.post("<?php echo BASE_URL; ?>ControlYAdministracion/Modales/EliminarProveedor.php", { id: id }, function(data) {
            $("#FormCajas").html(data);
