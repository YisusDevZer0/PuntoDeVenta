-- Tablas para el sistema de devoluciones - VERSIÓN CORREGIDA
...
INSERT INTO `Tipos_Devolucion` (`codigo`, `nombre`, `descripcion`, `color`, `requiere_autorizacion`) VALUES
...
-- Tabla de reportes de devoluciones (para cache de reportes)
...
CREATE INDEX idx_devoluciones_fecha_estatus ON Devoluciones(fecha, estatus);
CREATE INDEX idx_devoluciones_sucursal_fecha ON Devoluciones(sucursal_id, fecha);
CREATE INDEX idx_detalle_tipo_fecha ON Devoluciones_Detalle(tipo_devolucion, created_at);
CREATE INDEX idx_detalle_producto_cantidad ON Devoluciones_Detalle(producto_id, cantidad);
...
-- Verificar si las tablas Sucursales y Usuarios existen antes de crear las vistas
-- Vista para reportes de devoluciones (con verificación de tablas)
...
SELECT 
    ...
    COALESCE(u.TipoUsuario, 'Usuario') as usuario_tipo
FROM Devoluciones d
