...
try {
    ...
    header('Content-Disposition: attachment; filename="Reporte_Servicios_' . date('Y-m-d_H-i-s') . '.xls"');
    ...
    // Título del reporte
    echo '<div class="title">Reporte de Servicios</div>';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td>' . htmlspecialchars($row['Tipo_Servicio'] ?: 'Sin servicio') . '</td>';
    ...
    echo '</tr>';
    echo '<tr>';
    echo '<td style="font-weight: bold;">Total Servicios:</td>';
    echo '<td class="number">' . $total_servicios . '</td>';
    echo '</tr>';
    echo '<tr>';
    ...
    echo '</tr>';
    echo '<tr>';
    echo '<td style="font-weight: bold;">Promedio por Servicio:</td>';
    echo '<td class="currency">' . ($total_servicios > 0 ? $total_importe / $total_servicios : 0) . '</td>';
    echo '</tr>';
    echo '</table>';
    ...
    error_log('Error en exportar_reporte_servicios.php: ' . $e->getMessage());
