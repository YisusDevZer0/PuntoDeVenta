...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear') {
        ...
        $sql = "INSERT INTO pedidos (usuario_id, sucursal_id, estado, observaciones, tipo_origen) VALUES (?, ?, 'pendiente', ?, 'admin')";
        $stmt = $conn->prepare($sql);
    ...
    if ($accion === 'listar') {
        $sucursal_id = $row['Fk_Sucursal'];
        $sql = "SELECT p.id, d.cantidad, s.Nombre_Prod FROM pedidos p LEFT JOIN pedido_detalles d ON p.id = d.pedido_id LEFT JOIN Stock_POS s ON d.producto_id = s.ID_Prod_POS WHERE p.sucursal_id = ? ORDER BY p.id DESC LIMIT 50";
        $stmt = $conn->prepare($sql);
    ...
    if ($accion === 'detalle') {
        $id = intval($_POST['id']);
        $sql = "SELECT p.*, d.cantidad, s.Nombre_Prod FROM pedidos p LEFT JOIN pedido_detalles d ON p.id = d.pedido_id LEFT JOIN Stock_POS s ON d.producto_id = s.ID_Prod_POS WHERE p.id = ?";
        $stmt = $conn->prepare($sql);
