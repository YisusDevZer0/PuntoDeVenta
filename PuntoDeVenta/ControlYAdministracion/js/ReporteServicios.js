$(document).ready(function() {
    console.log("Inicializando DataTable para servicios...");
    ...
    var table = $('#tablaReporte').DataTable({
        ...
        "serverSide": false,
        "ajax": {
            "url": "Controladores/ArrayDeReportePorServicios.php",
            "type": "GET",
            "data": function(d) {
            ...
            "error": function(xhr, status, error) {
                ...
                console.error("URL llamada:", "Controladores/ArrayDeReportePorServicios.php");
                $('#loading-overlay').hide();
                Swal.fire({
                    icon: 'error',
    ...
    function actualizarEstadisticas(data) {
        if (data && data.length > 0) {
            var totalServicios = data.length;
            var totalVentas = 0;
            var totalClientes = 0;
            var totalImporte = 0;
            ...
            data.forEach(function(item) {
                ...
                // Contar clientes únicos (simplificado)
                totalClientes += parseInt(item.Numero_Ventas.replace(/,/g, ''));
            });
            
            var promedioVenta = totalServicios > 0 ? totalImporte / totalServicios : 0;
            
            $('#totalServicios').text(totalServicios);
            $('#totalVentas').text('$' + totalImporte.toLocaleString('es-MX', {minimumFractionDigits: 2}));
            $('#totalClientes').text(totalClientes.toLocaleString('es-MX'));
    ...
    window.exportarExcel = function() {
        ...
        var url = 'Controladores/exportar_reporte_servicios.php?fecha_inicio=' + fecha_inicio +
                  '&fecha_fin=' + fecha_fin +
                  '&sucursal=' + sucursal;
