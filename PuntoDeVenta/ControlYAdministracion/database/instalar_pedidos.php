<?php
// Script para instalar el sistema de pedidos
...
try {
    ...
    echo json_encode([
        'status' => 'info',
        'message' => 'Iniciando instalación del sistema de pedidos...'
    ]);
    
    // Leer el archivo SQL
    $sqlFile = __DIR__ . '/pedidos_schema.sql';
    
    if (!file_exists($sqlFile)) {
        throw new Exception("No se encontró el archivo pedidos_schema.sql");
    }
    ...
    if ($errorCount === 0 && count($tablesCreated) === count($tablesToCheck)) {
        echo json_encode([
            'status' => 'success',
            'message' => 'Sistema de pedidos instalado correctamente',
            'details' => [
        ...
        echo json_encode([
            'status' => 'warning',
            'message' => 'Instalación completada con algunos errores',
            'details' => [
                'queries_executed' => $successCount,
