<?php
// Script corregido para instalar el sistema de pedidos - Compatible con MariaDB
...
try {
    ...
    echo json_encode([
        'status' => 'info',
        'message' => 'Iniciando instalación del sistema de pedidos (versión corregida)...'
    ]);
    
    // Leer el archivo SQL corregido
    $sqlFile = __DIR__ . '/pedidos_schema_fixed.sql';
    
    if (!file_exists($sqlFile)) {
        throw new Exception("No se encontró el archivo pedidos_schema_fixed.sql");
    }
    ...
    if ($errorCount === 0 && count($tablesCreated) === count($tablesToCheck)) {
        echo json_encode([
            'status' => 'success',
            'message' => 'Sistema de pedidos instalado correctamente (versión corregida)',
            'details' => [
        ...
        echo json_encode([
            'status' => 'warning',
            'message' => 'Instalación completada con algunos errores (versión corregida)',
            'details' => [
                'queries_executed' => $successCount,
