-- =====================================================
-- SCRIPT SQL FINAL PARA SISTEMA DE RECORDATORIOS
-- Doctor Pez - Sistema de Punto de Venta
-- =====================================================

-- 1. Crear tabla principal de recordatorios (si no existe)
...
-- 2. Crear tabla de destinatarios (si no existe)
...
-- 3. Crear tabla de grupos (si no existe)
...
-- 4. Crear tabla de miembros de grupos (si no existe)
...
-- Crear grupo por defecto (solo si no existe)
INSERT IGNORE INTO `recordatorios_grupos` (`id_grupo`, `nombre_grupo`, `descripcion`, `activo`) 
VALUES (1, 'Todos los Usuarios', 'Grupo que incluye a todos los usuarios del sistema', 1);
...
SELECT 'Tablas de recordatorios verificadas:' as mensaje;

-- Verificar tabla principal
SELECT 
    CASE 
        WHEN COUNT(*) > 0 THEN 'recordatorios_sistema: OK'
        ELSE 'recordatorios_sistema: NO ENCONTRADA'
    END as estado_tabla
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
AND table_name = 'recordatorios_sistema';

-- Verificar tabla de destinatarios
SELECT 
    CASE 
        WHEN COUNT(*) > 0 THEN 'recordatorios_destinatarios: OK'
        ELSE 'recordatorios_destinatarios: NO ENCONTRADA'
    END as estado_tabla
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
AND table_name = 'recordatorios_destinatarios';

-- Verificar tabla de grupos
SELECT 
    CASE 
        WHEN COUNT(*) > 0 THEN 'recordatorios_grupos: OK'
        ELSE 'recordatorios_grupos: NO ENCONTRADA'
    END as estado_tabla
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
AND table_name = 'recordatorios_grupos';

-- Verificar tabla de logs
SELECT 
    CASE 
        WHEN COUNT(*) > 0 THEN 'recordatorios_logs: OK'
        ELSE 'recordatorios_logs: NO ENCONTRADA'
    END as estado_tabla
FROM information_schema.tables 
WHERE table_schema = DATABASE() 
AND table_name = 'recordatorios_logs';
