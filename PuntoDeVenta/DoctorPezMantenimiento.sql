...
-- Tiempo de generación: 12-04-2026 a las 17:03:51
...
--
-- Estructura de tabla para la tabla `GastosPOS_Audita`
--

CREATE TABLE `GastosPOS_Audita` (
  `ID_Audita_Gastos` int(11) NOT NULL,
  `ID_Gastos` int(10) UNSIGNED ZEROFILL NOT NULL,
  `Concepto_Categoria` varchar(200) NOT NULL,
  `Importe_Total` decimal(50,2) NOT NULL,
  `Fk_sucursal` int(12) NOT NULL,
...
--
-- Estructura de tabla para la tabla `Inventario_Turnos_Config_Empleado`
--

CREATE TABLE `Inventario_Turnos_Config_Empleado` (
  `ID_Config` int(11) NOT NULL,
...
--
-- Estructura de tabla para la tabla `Inventario_Turnos_Config_Sucursal`
--

CREATE TABLE `Inventario_Turnos_Config_Sucursal` (
  `ID_Config` int(11) NOT NULL,
...
--
-- Estructura de tabla para la tabla `Inventario_Turnos_Periodos`
--

CREATE TABLE `Inventario_Turnos_Periodos` (
...
CREATE TRIGGER `tr_pago_servicio_delete` AFTER DELETE ON `PagosServicios` FOR EACH ROW BEGIN
            ...
            -- Solo la comisión afecta la caja; el costo queda en PagosServicios para cortes/reportes
...
CREATE TRIGGER `tr_pago_servicio_insert` AFTER INSERT ON `PagosServicios` FOR EACH ROW BEGIN
            ...
            -- Solo la comisión suma a la caja; el costo (NEW.costo) no incrementa Valor_Total_Caja
            SELECT IFNULL(Comision, 0.00) INTO v_comision
            FROM ListadoServicios
...
CREATE TRIGGER `tr_pago_servicio_update` AFTER UPDATE ON `PagosServicios` FOR EACH ROW BEGIN
            ...
            -- Ajustar caja solo por comisión; cambios de costo sin cambio de servicio no mueven la caja
...
--
-- Estructura de tabla para la tabla `Solicitudes_Traspaso_Sucursales`
--

CREATE TABLE `Solicitudes_Traspaso_Sucursales` (
  `ID_Solicitud` int(11) NOT NULL,
  `Fk_sucursal_solicitante` int(11) NOT NULL COMMENT 'Sucursal que pide el producto (quien solicita)',
  `Fk_sucursal_solicitada` int(11) NOT NULL COMMENT 'Sucursal que tiene el producto (a quien se le solicita)',
  `ID_Prod_POS` int(11) NOT NULL,
  `Cod_Barra` varchar(100) DEFAULT NULL,
  `Nombre_Prod` varchar(255) DEFAULT NULL,
  ...
  `Observaciones` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Solicitudes de traspaso entre sucursales; visible para Admin y sucursal solicitada';
    ...
    ELSE
        ...
        IF v_a_restar_stock > 0 THEN
            INSERT INTO inventory_movements (
                ...
                'exit', 'sale', CAST(NEW.Venta_POS_ID AS CHAR),
                -v_a_restar_stock, v_stock, v_stock - v_a_restar_stock,
                'Venta', NEW.AgregadoPor
            );
        END IF;
...
--
-- Indices de la tabla `GastosPOS_Audita`
--
ALTER TABLE `GastosPOS_Audita`
  ADD PRIMARY KEY (`ID_Audita_Gastos`),
  ADD KEY `idx_gastos_audita_caja` (`Fk_Caja`),
  ADD KEY `idx_gastos_audita_fecha` (`AgregadoEl`),
  ADD KEY `idx_gastos_audita_id_gastos` (`ID_Gastos`);
...
--
-- Indices de la tabla `Inventario_Turnos_Config_Empleado`
--
ALTER TABLE `Inventario_Turnos_Config_Empleado`
  ADD PRIMARY KEY (`ID_Config`),
  ADD UNIQUE KEY `uk_usuario_sucursal` (`Fk_usuario`,`Fk_sucursal`),
  ADD KEY `idx_usuario` (`Fk_usuario`),
  ADD KEY `idx_sucursal` (`Fk_sucursal`);

--
-- Indices de la tabla `Inventario_Turnos_Config_Sucursal`
--
ALTER TABLE `Inventario_Turnos_Config_Sucursal`
  ADD PRIMARY KEY (`ID_Config`),
  ADD UNIQUE KEY `uk_sucursal` (`Fk_sucursal`),
  ADD KEY `idx_activo` (`Activo`);
...
--
-- Indices de la tabla `Inventario_Turnos_Periodos`
--
ALTER TABLE `Inventario_Turnos_Periodos`
  ADD PRIMARY KEY (`ID_Periodo`),
  ADD KEY `idx_sucursal_fechas` (`Fk_sucursal`,`Fecha_Inicio`,`Fecha_Fin`),
  ADD KEY `idx_activo` (`Activo`);
...
ALTER TABLE `inventory_movements`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_inv_mov_sucursal` (`Fk_sucursal`),
  ADD KEY `idx_inv_mov_created` (`created_at`),
  ADD KEY `idx_inv_mov_reference` (`reference_type`,`reference_id`),
  ADD KEY `idx_inv_mov_cod_barra` (`Cod_Barra`);
...
--
-- Indices de la tabla `Solicitudes_Traspaso_Sucursales`
--
ALTER TABLE `Solicitudes_Traspaso_Sucursales`
  ADD PRIMARY KEY (`ID_Solicitud`),
  ADD KEY `idx_sucursal_solicitada` (`Fk_sucursal_solicitada`),
  ADD KEY `idx_sucursal_solicitante` (`Fk_sucursal_solicitante`),
  ADD KEY `idx_estatus` (`Estatus`),
  ADD KEY `idx_solicitado_el` (`Solicitado_el`);
...
ALTER TABLE `Stock_POS`
  ...
  ADD KEY `idx_stock_pos_sucursal_nombre` (`Fk_sucursal`,`Nombre_Prod`(100)),
  ADD KEY `idx_stock_pos_sucursal_existencias` (`Fk_sucursal`,`Existencias_R`,`Min_Existencia`),
  ADD KEY `idx_stock_pos_fk_sucursal` (`Fk_sucursal`);
...
--
-- AUTO_INCREMENT de la tabla `GastosPOS_Audita`
--
ALTER TABLE `GastosPOS_Audita`
  MODIFY `ID_Audita_Gastos` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT de la tabla `Inventario_Turnos_Config_Empleado`
--
ALTER TABLE `Inventario_Turnos_Config_Empleado`
  MODIFY `ID_Config` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de la tabla `Inventario_Turnos_Config_Sucursal`
--
ALTER TABLE `Inventario_Turnos_Config_Sucursal`
  MODIFY `ID_Config` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT de la tabla `Inventario_Turnos_Periodos`
--
ALTER TABLE `Inventario_Turnos_Periodos`
...
--
-- AUTO_INCREMENT de la tabla `Solicitudes_Traspaso_Sucursales`
--
ALTER TABLE `Solicitudes_Traspaso_Sucursales`
